...
const googleLoginData = require("./config/secrets");
...
app.get("/logout", function(req, res) {
  ...
  console.log("loging out sir");
  req.logout();
  res.redirect("/login.html");
});
app.use(fileNotFound); // otherwise not found

const PORT = process.env.PORT || 50612;
...
function page_redirection_checking(req, res, next) {
  ...
  const dbCheck =
    "SELECT * FROM flashcards WHERE Google_id = " + req.user.id + " ";
...
function InsertNewUser(profile) {
  ...
  db.run(
    dbinsert,
    {
      ...
      $g_name: profile.name.givenName
    },
    tableInsertionCallback
  );
...
function reachDatabase(english_txt, other_language_txt, req, res) {
  let respondObject = {};
  const dbStore =
    "INSERT INTO Flashcards VALUES($user_id, $eng_txt, $other_lang_txt, 0, 0)";
  ...
  db.run(
    dbStore,
    {
      $user_id: req.user.id,
      $eng_txt: english_txt,
      $other_lang_txt: other_language_txt
    },
    tableInsertionCallback
  );
